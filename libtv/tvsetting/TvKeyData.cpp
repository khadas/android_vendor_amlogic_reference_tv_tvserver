...
static int write_partition_raw(const char *partition, const void *data, const int size)
{
    int fd = 0;
...
static int read_partition_raw(const char *partition, char *data, const int size)
{
    int fd = 0;
    int len = 0;
...
int ReadKeyData(const char *key_name, unsigned char data_buf[], int rd_size)
{
    ...
    if (write_partition_raw(CS_KEY_DATA_ATTACH_DEV_PATH, "1", strlen("1"))) {
        LOGE("%s, attach failed!\n", __FUNCTION__);
        return -__LINE__;
    }

    if (write_partition_raw(CS_KEY_DATA_NAME_DEV_PATH, key_name, strlen(key_name))) {
        LOGE("%s, name failed!\n", __FUNCTION__);
        return -__LINE__;
    }

    ret = read_partition_raw(CS_KEY_DATA_READ_DEV_PATH, (char*)data_buf, rd_size);
    if (ret != rd_size) {
...
static int GetSSMMacAddressStartWorkEnableCFG()
{
    ...
    config_value = config_get_str(CFG_SECTION_TV, CS_MAC_ADDRESS_STARTWORK_EN_CFG, "null");
    if (strcmp(config_value, "null") == 0) {
        LOGD(
...
static int KillMacAddressStartWorkThread()
{
    ...
    LOGD("%s, kill the mac address start work thread success.\n", __FUNCTION__);
...
int CreateMacAddressStartWorkThread()
{
    ...
    LOGD("%s, create channel select thread success.\n", __FUNCTION__);
               ...
               LOGD("%s: load amdolby_vision EDID!\n", __FUNCTION__);
                if (loadNum%2 != 0) {
                    ...
                    if (isLoadDLGEdid) {
                        ...
                        sprintf(edidFilePath, "%s/port%d_dv_dlg.bin", edidDirPath, loadNum/2);
                    } else {
                ...
                if (loadNum%2 != 0) {
                    ...
                    if (isLoadDLGEdid) {
                        ...
                        sprintf(edidFilePath, "%s/port%d_dlg.bin", edidDirPath, loadNum/2);
                    } else {
