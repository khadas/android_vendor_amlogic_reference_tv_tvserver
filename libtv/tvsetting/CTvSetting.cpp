...
{
    return SSMSaveBurnWriteCharacterChar(CC_DEF_CHARACTER_CHAR_VAL);
}

int SSMSaveBurnWriteCharacterChar(int rw_val)
{
    return TVSSMWriteNTypes(SSM_RSV_W_CHARACTER_CHAR_START, 1, rw_val);
}

int SSMReadBurnWriteCharacterChar()
{
    int tmp_val = 0;
...
int SSMReadVDac3DValue()
{
    ...
    if (TVSSMReadNTypes(SSM_RW_VDAC_3D_START, 1, &tmp_val) < 0) {
        return 0;
    }
...
int SSMDeviceMarkCheck()
{
    ...
    if (SSMReadBurnWriteCharacterChar() != CC_DEF_CHARACTER_CHAR_VAL) {
        SSMSaveBurnWriteCharacterChar(CC_DEF_CHARACTER_CHAR_VAL);
    }
