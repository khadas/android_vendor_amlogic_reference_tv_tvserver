...
int CTvin::VDIN_StartDec ( const struct tvin_parm_s *vdinParam )
{
    ...
    if (mSupportResman) {
        ...
        //Underscores in xx vdin_secure & vdin_mm_size  newly added to pass typo censorship
        //when vdin add the corresponding properties and then modify
        property_get("vendor.media.vdin.vdin_secure", value, "0");
        res_status.v.query.value = atoi(value);
        property_get("vendor.media.vdin.waitnumber", value, "100");
        try_count  = atoi(value);

        bool isAtv = false;
        if (TVIN_PORT_CVBS0 <= m_tvin_param.port && m_tvin_param.port <= TVIN_PORT_CVBS7) {
            isAtv = true;
            LOGE ("isatv true\n");
        }
        ...
        sprintf(argsize, "size:%d", vdin_mm_size);
        ...
        {
            if (res_status.v.query.avail > vdin_mm_size)
                break;
            res_status.v.query.value = atoi(value);
            usleep(100000);
            try_count --;
        }
        LOGI ( "avail 0x%x vdin_mm_size 0x%x\n",res_status.v.query.avail,vdin_mm_size);
#endif
...
int CTvin::AFE_SetVGAAutoAdjust ( void )
{
    int rt = -1;
    ...
    if ( ( Status == TVAFE_CMD_STATUS_IDLE ) || ( Status == TVAFE_CMD_STATUS_SUCCESSFUL ) ) {
        ...
        LOGW ( "s%: TVIN_IOC_G_AFE_CMD_STATUS failed!\n", __FUNCTION__ );
        return -1;
    }
...
int CTvin::TvinApi_SetCompPhaseEnable ( int enable )
{
    int ret = -1;
    if ( enable == 1 ) {
        ret = tvWriteSysfs (PATH_AFE_DPHASE_EN_49, "Y" );
...
void CTvin::tvin_onResPreemptCallback(void* param)
{
#if RESMAN_SUPPORT_PREEMPT
...
#if RESMAN_SUPPORT_PREEMPT
    if (!mOnPreemptRegistered) {
         resman_register(mResmanFd, tvin_onResPreemptCallback, this);
         mOnPreemptRegistered = true;
    }
#endif
...
#endif
        LOGE("%s:res_acquire(handle=%d,res=%d) failed!!!\n",
