            ...
            if (npd != NULL) {
                switch (npd->newts->type) {
                    case AM_SCAN_TS_ANALOG:
                        if (npd->result->start_para->atv_para.mode)
                    ...
                    case AM_SCAN_TS_DIGITAL:
                        if (npd->result->start_para->dtv_para.mode)
...
int CTvScanner::getAtscChannelPara(int antennaType, Vector<sp<CTvChannel> > &vcp)
{
    switch (antennaType) { //region name should be remove to config file and read here
    case 1:
