...
CTvRecord::CTvRecord()
{
   ...
   int cnt;
    ...
    memset(&wrap_recorder, 0, sizeof(wrap_recorder ));
...
int CTvRecord::setTimeShiftMode(bool enable, int duration, int size)
{
    ...
    if (mIsTsplayer == true) {
      ...
      rec_open_params.is_timeshift = (enable == true ? DVR_TRUE : DVR_FALSE);
      rec_open_params.max_time = (enable == true ? duration * 1000 : 0);
      rec_open_params.max_size = (enable == true ? size : 0);
      if (true == enable) {//is timeshift
    ...
    mRecParam.is_timeshift = (enable == true ? true : false);
    mRecParam.total_time = (enable == true ? duration : 0);
    mRecParam.total_size = (enable == true ? size : 0);
#endif
    return 0;
}
...
int CTvRecord::start(const char *param)
{
    ...
    if (mIsTsplayer == true) {
        ...
        char location[DVR_MAX_LOCATION_SIZE] = {0};
        strcpy(location,rec_open_params.location);
        snprintf(rec_open_params.location, DVR_MAX_LOCATION_SIZE,
              "%s/%s", location, DEFAULT_TIMESHIFT_BASENAME);
