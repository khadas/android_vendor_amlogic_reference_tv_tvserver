...
#define DEFAULT_TIMESHIFT_BASENAME   "timeshift"
...
int CAv::SwitchTSAudio(int apid, int afmt)
{
    ...
    am_tsplayer_audio_params parm;
    am_tsplayer_result ret = AM_TSPLAYER_OK;

    parm.codectype = toTsPlayerAudioFormat(afmt);
    parm.pid = apid;
    ...
    if (mAdState) {
        LOGD("%s: AmTsPlayer_setADParams", __FUNCTION__);
        ret = AmTsPlayer_setADParams(mSession, &mAdParams);
        if (ret != AM_TSPLAYER_OK) {
          LOGD("%s: ret is %d\n", __FUNCTION__, ret);
          return -1;
        }
    ...
    ret = AmTsPlayer_setAudioParams(mSession, &parm);
    if (ret != AM_TSPLAYER_OK) {
        LOGD("%s: ret is %d\n", __FUNCTION__, ret);
        return -1;
    }
...
int CAv::SetADAudio(unsigned int enable, int apid, int afmt)
{
    ...
    if (mIsTsplayer == false) {
        ...
        if (mIsTimeshift == false) {
            //for live mode
            //save AD params and set to tsplayer at startTS or SwitchTSAudio
            LOGD("%s: line %d, mAdValid %d", __FUNCTION__, __LINE__, mAdState);
            if (VALID_PID(apid)) {
                mAdParams.codectype = toTsPlayerAudioFormat(afmt);
                mAdParams.pid = apid;
                mAdState = 1;
            } else {
                mAdParams.codectype = AV_AUDIO_CODEC_MAX;
                mAdParams.pid = 0;
                mAdState = 0;
            }
        } else {
...
int CAv::StartTS(unsigned short vpid, unsigned short apid, unsigned short pcrid, int vfmt, int afmt)
{
    ...
    am_tsplayer_video_params v_parm;
    am_tsplayer_audio_params a_parm;
    ...
    v_parm.codectype = toTsPlayerVideoFormat(vfmt);
    v_parm.pid = vpid;
    if (mSession != INVALID_PLAYER_HDLE && VALID_PID(v_parm.pid)) {
        ret = AmTsPlayer_setVideoParams(mSession, &v_parm);
        if (ret != AM_TSPLAYER_OK) {
            LOGD("%s: ret is %d\n", __FUNCTION__, ret);
            return -1;
        }
    ...
    a_parm.pid = apid;
    if (mSession != INVALID_PLAYER_HDLE)
        ret = AmTsPlayer_setAudioParams(mSession, &a_parm);
    if (ret != AM_TSPLAYER_OK) {
        LOGD("%s: ret is %d\n", __FUNCTION__, ret);
        return -1;
    }
...
int CAv::StopTS()
{
    ...
    if (mIsTimeshift == false) {
        ...
        LOGD("%s: -----stop timeshift when stop ts--when in timeshift mode--\n", __FUNCTION__);
        stopTimeShift();
    }
...
int CAv::EnableVideoBlackout()
{
    ...
    if (mSession != INVALID_PLAYER_HDLE) {
        ...
        LOGD("%s: mTsPlayer_setVideoBlackOut\n", __FUNCTION__);
    }
    if (ret != AM_TSPLAYER_OK) {
        LOGD("%s: ret is %d\n", __FUNCTION__, ret);
        return -1;
...
int CAv::EnableVideoNow(bool IsShowTestScreen)
{
    ...
    if (IsShowTestScreen) {
        LOGD("%s: enableVideoWithBlackColor SwitchSourceTime = %fs", __FUNCTION__,getUptimeSeconds());
...
{
    LOGD("---set dmx source hiu--||%s", __FUNCTION__);
#ifdef SUPPORT_ADTV
    if (mIsTsplayer == false) {
...
int CAv::EnableVideoAuto()
{
    ...
    mVideoLayerState = VIDEO_LAYER_ENABLE;
    LOGD("%s: enableVideoWithBlackColor\n", __FUNCTION__);
    SetVideoScreenColor(VIDEO_LAYER_BLACK); // Show black with vdin0, postblending disabled
