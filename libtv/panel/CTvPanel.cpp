...
CTvPanel *CTvPanel::mInstance;

CTvPanel *CTvPanel::getInstance()
{
    if (NULL == mInstance) mInstance = new CTvPanel();
    return mInstance;
}
...
int CTvPanel::PANEL_SIZE_CHOICE(){
    ...
    int demura_offset = (((int)key_data[19]<<24)+((int)key_data[18]<<16)+((int)key_data[17]<<8)+(int)key_data[16]);
    ...
    int p_gamma_size = (int) ((key_data[55]<<24)+(key_data[54]<<1)+(key_data[53]<<8)+key_data[52]);
...
#define   RES_SIZE_50_PANEL   130351
...
int CTvPanel::PANEL_50INX_data_Conversion(const char *partition_name_Table_data,const char *partition_name_Register) {
    ...
    /*
    unsigned char lut1[RES_SIZE_50_PANEL] = {0x0};
    unsigned char lut2[RES_SIZE_50_PANEL] = {0x0};
    unsigned char lut3[RES_SIZE_50_PANEL] = {0x0};
    unsigned char lut4[RES_SIZE_50_PANEL] = {0x0};
    unsigned char lut5[RES_SIZE_50_PANEL] = {0x0};
    */
    unsigned char *lut1 = NULL;
    lut1 = (unsigned char *)malloc(RES_SIZE_50_PANEL*sizeof(unsigned char));
    if (lut1 == NULL) {
        ALOGE("%s,Malloc memory failed!\n", __FUNCTION__);
        free(all_buffer);
        all_buffer = NULL;
    ...
    lut2 = (unsigned char *)malloc(RES_SIZE_50_PANEL*sizeof(unsigned char));
    if (lut2 == NULL) {
        ALOGE("%s,Malloc memory failed!\n", __FUNCTION__);
        free(all_buffer);
        all_buffer = NULL;
    ...
    lut3 = (unsigned char *)malloc(RES_SIZE_50_PANEL*sizeof(unsigned char));
    if (lut3 == NULL) {
        ALOGE("%s,Malloc memory failed!\n", __FUNCTION__);
        free(all_buffer);
        all_buffer = NULL;
    ...
    lut4 = (unsigned char *)malloc(RES_SIZE_50_PANEL*sizeof(unsigned char));
    if (lut4 == NULL) {
        ALOGE("%s,Malloc memory failed!\n", __FUNCTION__);
        free(all_buffer);
        all_buffer = NULL;
    ...
    lut5 = (unsigned char *)malloc(RES_SIZE_50_PANEL*sizeof(unsigned char));
    if (lut5 == NULL) {
        ALOGE("%s,Malloc memory failed!\n", __FUNCTION__);
        free(all_buffer);
        all_buffer = NULL;
...
#define   RES_SIZE_65_PANEL   130351
int CTvPanel::PANEL_65D02_H_data_Conversion(const char *partition_name_Table_data,const char *partition_name_Register){
    ...
    //char plane1[RES_SIZE_65_PANEL];
    char *plane1 = NULL;
    plane1 = (char *)malloc(RES_SIZE_65_PANEL*sizeof(char));
    if (plane1 == NULL) {
        LOGE("%s,Malloc memory failed!\n", __FUNCTION__);
        free(buffer);
        buffer = NULL;
    ...
    //char plane2[RES_SIZE_65_PANEL];
    char *plane2 = NULL;
    plane2 = (char *)malloc(RES_SIZE_65_PANEL*sizeof(char));
    if (plane2 == NULL) {
        LOGE("%s,Malloc memory failed!\n", __FUNCTION__);
        free(buffer);
        buffer = NULL;
    ...
    //char plane3[RES_SIZE_65_PANEL];
    char *plane3 = NULL;
    plane3 = (char *)malloc(RES_SIZE_65_PANEL*sizeof(char));
    if (plane3 == NULL) {
        LOGE("%s,Malloc memory failed!\n", __FUNCTION__);
        free(buffer);
        buffer = NULL;
    ...
    for (int j = 0; j < 3; j++) {
        ...
        for (int i = 0; i < 892; i++) {
            if (i == 891) {
                ...
                if (ret < 0) {
                    LOGE("%s, flash_demura_1024 Failed!\n", __FUNCTION__);
                    free(Demura_Data);
                    Demura_Data = NULL;
                    free(flash_demura);
                    flash_demura = NULL;
...
int CTvPanel::PANEL_SPI_PGAMMA(int spi_offset, int spi_len ,const char *spi_bin_path) {
    ...
    unsigned char crc_height = (unsigned char)(crc_ret >> 8);
    if (crc_low == buffer[41] && crc_height == buffer[40]) {
...
int CTvPanel::PANEL_SPI_PGAMMA_SPEC(int spi_offset, int spi_len ,const char *spi_bin_path, const char *default_bin_path) {
    ...
    unsigned char crc_low =(unsigned char) (crc_ret & 0x00FF);
    unsigned char crc_height = (unsigned char)(crc_ret >> 8);
    if (crc_low == buffer[24] && crc_height == buffer[23]) {
        LOGE("%s,Crc check pass!\n", __FUNCTION__);
    } else {
        LOGE("%s,Crc check not pass,use defult gamma data!\n", __FUNCTION__);
        PANEL_REMOVE_BIN_FILE(1);
...
int CTvPanel::PANEL_SPI_PGAMMA_SPEC_65D02_3(int spi_offset, int spi_len ,const char *spi_bin_path, const char *default_bin_path) {
    ...
    unsigned char crc_low =(unsigned char) (crc_ret & 0x00FF);
    unsigned char crc_height = (unsigned char)(crc_ret >> 8);
    if (crc_low == flash_crc_low && crc_height == flash_crc_high) {
        LOGE("%s,Crc check pass!\n", __FUNCTION__);
    } else {
        PANEL_REMOVE_BIN_FILE(1);
        return -1;
