...
void DroidTvServiceIntf::onTvEvent(const CTvEv &ev)
{
    ...
    case CTvEv::TV_EVENT_SIGNAL_DETECT: {
    ...
    case CTvEv::TV_EVENT_RRT: {
        ...
        if (pRrtEvent->mRrtInfo) {
            ...
            hidlParcel.bodyInt.resize(7);
            hidlParcel.bodyString.resize(4);
            hidlParcel.bodyInt[0] = pRrtEvent->mRrtInfo->rrt_section_count;
            hidlParcel.bodyInt[1] = pRrtEvent->mRrtInfo->table_id;
            hidlParcel.bodyInt[2] = pRrtEvent->mRrtInfo->rating_region;
            hidlParcel.bodyInt[3] = pRrtEvent->mRrtInfo->version_number;
            hidlParcel.bodyInt[4] = pRrtEvent->mRrtInfo->dimension_defined;
            hidlParcel.bodyInt[5] = pRrtEvent->mRrtInfo->graduated_scale;
            hidlParcel.bodyInt[6] = pRrtEvent->mRrtInfo->values_defined;
            hidlParcel.bodyString[0]= pRrtEvent->mRrtInfo->rating_region_name;
            hidlParcel.bodyString[1]= pRrtEvent->mRrtInfo->dimension_name;
            hidlParcel.bodyString[2]= pRrtEvent->mRrtInfo->abbrev_rating_value_text;
            hidlParcel.bodyString[3]= pRrtEvent->mRrtInfo->rating_value_text;
            mNotifyListener->onEvent(hidlParcel);
        }
        break;
    }
...
int DroidTvServiceIntf::loadEdidData(int32_t isNeedBlackScreen, int32_t isDolbyVisionEnable) {
    return mpTv->LoadEdidData(isNeedBlackScreen, isDolbyVisionEnable, 0);
}
...
int DroidTvServiceIntf::atvManualScan(int32_t startFreq, int32_t endFreq, int32_t videoStd, int32_t audioStd) {
    return mpTv->atvManualScan(startFreq, endFreq, videoStd, audioStd);
}
...
int DroidTvServiceIntf::Tv_GetDLGEnable() {
    return mpTv->Tv_GetDLGEnable();
}

int DroidTvServiceIntf::SetVRREnable(bool isEnable) {
    return mpTv->TV_SetVRREnable(isEnable);
}

int DroidTvServiceIntf::GetVRREnable() {
    return mpTv->TV_GetVRREnable();
}


int DroidTvServiceIntf::GetVRRMode()
{
    return mpTv->getVrrMode();
}

int DroidTvServiceIntf::GetAllmInfo(void)
{
    return mpTv->getAllmInfo();
}
...
int DroidTvServiceIntf::updateRRT(int freq, int module, int mode) {
    //return mpTv->Tv_RrtUpdate(freq, module, mode);
    return 0;
}

int DroidTvServiceIntf::updateEAS(int freq __unused, int module __unused, int mode __unused) {
...
int DroidTvServiceIntf::getBasicVdecStatusInfo(int vdecId, int &decode_time_cost, int &frame_width, int &frame_height, int &frame_rate, int &error_count, int &frame_count, int &error_frame_count, int &drop_frame_count, int &double_write_mode) {
...
int DroidTvServiceIntf::processCmd(const Parcel &p) {
    ...
    switch (cmd) {
        ...
        case ATV_SCAN_MANUAL: {
            ...
            ret = mpTv->atvManualScan(startFreq, endFreq, videoStd, audioStd);
        ...
        case SET_FRONTEND_PARA: {
            frontend_para_set_t feParms;
            memset(&feParms,0,sizeof(feParms));
        ...
        case PLAY_PROGRAM: {
            ...
            if (mode == TV_FE_ANALOG) {
                ...
                int audioCompetition = p.readInt32();
                mpTv->playAtvProgram(freq, videoStd, audioStd, vfmt, soundsys, fineTune, audioCompetition);
            } else {
                int para1 = p.readInt32();
                int para2 = p.readInt32();
                int vid = p.readInt32();
                ...
                int pcr = p.readInt32();
                int audioCompetition = p.readInt32();
                mpTv->playDtvProgram(mode, freq, para1, para2, vid, vfmt, aid, afmt, pcr, audioCompetition);
            }
            break;
        }
        ...
        case PLAY_PROGRAM_2: {
            ...
            int pcr = p.readInt32();
            int audioCompetition = p.readInt32();
            mpTv->playDtvProgram(feparas.string(), vid, vfmt, aid, afmt, pcr, audioCompetition);
            break;
        }
        case TV_SCAN_2: {
            String8 feparas(p.readString16());
